...
<title>Lupa Password · SPK Jurusan SMKN 2 Jember</title>
...
.brand-logo { width: 48px; height: 48px; background: #fff; border-radius: 50%; display: flex; align-items: center; justify-content: center; padding: 6px; box-shadow: 0 4px 16px rgba(0,0,0,.2); flex-shrink: 0; overflow: hidden; }
...
.input-wrap svg { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); width: 15px; height: 15px; color: rgba(255,255,255,.3); pointer-events: none; }
input[type="email"] { width: 100%; background: rgba(255,255,255,.06); border: 1.5px solid rgba(255,255,255,.1); border-radius: 10px; padding: 12px 14px 12px 36px; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 14px; color: var(--white); outline: none; transition: border-color .2s, background .2s; }
...
.btn-submit::after { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(255,255,255,.12), transparent); }
.btn-submit:hover { transform: translateY(-1px); box-shadow: 0 10px 28px rgba(37,99,235,.5); }
.back-link { display: block; text-align: center; font-size: 13px; color: var(--gold); text-decoration: none; font-weight: 600; margin-top: 16px; }
...
<div class="left">
    ...
    <div class="brand">
        <div class="brand-logo">
            <img src="{{ asset('Assets/Logo_SMKN2Jember.png') }}" alt="Logo">
        </div>
    ...
    <div class="hero-label"><span></span> Reset Akses</div>
    <h1 class="hero-title">Lupa<br><em>Password?</em></h1>
    <p class="hero-desc">Masukkan email yang terdaftar dan kami akan mengirimkan link untuk mereset password akunmu di sistem SPK SMKN 2 Jember.</p>
</div>
<div class="vline"></div>
<div class="right">
    <div class="card">
        <div class="card-top">
            ...
            <p>Masukkan email yang terdaftar di sistem</p>
        </div>
        @if (session('status'))
